...
surface.CreateFont("RogueTitle", {font = "Quicksand Regular", size = 58, extended = true, antialias = true,})
		...
		table.insert(CommandTable, {
			...
			Func = function(cmdPly)
				if IsValid(cmdPly) and !cmdPly:IsBot() then
					...
					SetClipboardText(cmdPly:SteamID() .. " & " .. cmdPly:SteamID64() .. " ")
...
local function mainPaint(self, w, h)
	...
	if Configuration.SteamName then
		RandName = ply.RandomName or ply:Name()
	else
		RandName = ply.RandomName or ply:Nick()
	end 
	...
	local DisplayColor = DisplaySetting and DisplaySetting.DisplayColor or Color( 255,255,255 )

	if Configuration.DarkRP && Configuration.ShowTeam then
		draw.DrawText( ply:getDarkRPVar("job"), "Rogue24", w / 2, 7, White, TEXT_ALIGN_CENTER)
	end
	...
	if Configuration.ShowRank then
		if ply:GetRogueNetBool("Incognito") then
			...
			draw.DrawText( DisplayName, "Rogue24", w / 2 + w / 4 - 320, 7, DisplayColor, TEXT_ALIGN_CENTER)
		end
	end
...
local function createScoreboard()
	...
	function Main:Paint( w, h )
		...
		surface.SetMaterial( Material("materials/tylogo.png") )
		surface.DrawTexturedRect( w / 2 - 75, 35, 150,150 )

		draw.DrawText(  Configuration.ServerTitle , "RogueTitle", w / 2 , 100 + 65 + 20, Configuration.ServerTitleColor, TEXT_ALIGN_CENTER)
		draw.DrawText(  "" , "Rogue24", w / 2 , 145 + 60 + 30, Configuration.ServerTitleColor, TEXT_ALIGN_CENTER)
